...
// Adjusted to match current controller implementation (uses normalizedSelectedDate and calendar.dataset.selectedDate)
test('availability calendar flow shows the placeholder payment system after selecting a date', () => {
  assert.ok(appControllerSource.includes('calendar.dataset.selectedDate'), 'Selecting a day should store the chosen date on the calendar state.');
  ...
  // The implementation does not include a literal "PIVOT Payment System" label; check for payment panel header or reveal logic instead
